...
public class BFSShortestPath {
	...
	static class Node {
        ...
        Node(int x, int y,int updatedHealth,int maxNegative,Node prevN) {
            this.x = x;
            this.y = y;
    ...
    public static  List<Node> getNeighbors(int[][] dungeon, Node node, int[][] minHealthChat){
        List<Node> neighbors = new ArrayList<Node>();
        
        if(isValidPoint(dungeon, node.x + 1, node.y)) {
        	int nextMaxNegative = 0;
        	
        	if((dungeon[node.x + 1][node.y]+node.updatedHealth) < node.maxNegative) {
        		nextMaxNegative = dungeon[node.x + 1][node.y]+node.updatedHealth;
        	}
        	else {
        		nextMaxNegative = node.maxNegative;
        	}
//        	System.out.println(nextMaxNegative);
//        	System.out.println(minHealthChat[node.x + 1][node.y]);
        	if( (minHealthChat[node.x + 1][node.y] < nextMaxNegative) || (dungeon[node.x + 1][node.y] >=0) || 
        		(dungeon.length - 1 == node.x + 1)&& (dungeon[dungeon.length - 1].length -1 == node.y)) {
        		neighbors.add(new Node(node.x + 1, node.y,(dungeon[node.x + 1][node.y]+node.updatedHealth),nextMaxNegative,node));
        		minHealthChat[node.x + 1][node.y] = nextMaxNegative;
        	}
        }
                
        if(isValidPoint(dungeon, node.x, node.y + 1)) {
        	int nextMaxNegative = 0;
        	if((dungeon[node.x][node.y+1]+node.updatedHealth) < node.maxNegative) {
        		nextMaxNegative = dungeon[node.x][node.y+1]+node.updatedHealth;
        	}
        	else {
        		nextMaxNegative = node.maxNegative;
        	}
        	if((minHealthChat[node.x][node.y+1] < nextMaxNegative) || (dungeon[node.x][node.y+1] >=0) || 
            		(dungeon.length - 1 == node.x)&& (dungeon[dungeon.length - 1].length -1 == node.y+1)) {
        		neighbors.add(new Node(node.x, node.y + 1,(dungeon[node.x][node.y + 1]+node.updatedHealth),nextMaxNegative,node));
        		minHealthChat[node.x][node.y+1] = nextMaxNegative;
        	}
        }
    ...
    public static int calculateMinimumHP(int[][] dungeon) {
        ...
        int [][] minHealthChat = new int[Row+1][Col+1];
        for(int i = 0 ; i <= Row;i++) {
        	for( int j=0; j <= Col; j++) {
        		minHealthChat[i][j] = Integer.MIN_VALUE;
        	}
        }
        List<Node> queue = new ArrayList<Node>();
        int maxNegative = 0;
        if(dungeon[0][0]<0) {
        	maxNegative = maxNegative + dungeon[0][0];
        }
        else{
        	maxNegative = 0;
        }
        queue.add(new Node(0, 0,dungeon[0][0],maxNegative,null));
        minHealthChat[0][0]  = maxNegative;
        
        while(!queue.isEmpty()){
            Node current = queue.remove(0);
            System.out.println("Value at : ["+current.x+"]["+current.y+"] is: "+current.updatedHealth+" and minHealth is: "+current.maxNegative);
//            System.out.println(minHealthChat[current.x + 1][current.y]);
            if((Row == current.x)&&(Col == current.y)) { 
            	if(minhealth == 0) {
            		minhealth = current.maxNegative;
            	}
            	if(current.maxNegative >=0) {
            		minhealth = 1;
            	}
                if(current.maxNegative > minhealth ) {
                	minhealth = current.maxNegative;
                }
            }
            
            List<Node> neighbors = getNeighbors(dungeon, current,minHealthChat);
            
            queue.addAll(neighbors);
        }
                
        if(minhealth <1){
        	minhealth = 0-minhealth +1;
        }
...
//							{0,0}};
		...
		int [][] dungeon = {{2,-8,-79,-88,-12,-87,-5,-56,-55,-42,18,-91,1,-30,-36,42,-96,-26,-17,-69,38,18,44,-58,-33,20,-45,-11,11,15,-40,-92,-62,-51,-23,20,-86,-2,-90,-64,-100,-42,-16,-55,29,-62,-81,-60,7,-5,31,-7,40,19,-53,-81,-77,42,-87,37,-43,37,-50,-21,-86,-28,13,-18,-65,-76},
...
//							 {-10,0}};
		...
		int [][] dungeon2 = {{29,-78,-52,-1,-38,6,24,-45,35,-29},
